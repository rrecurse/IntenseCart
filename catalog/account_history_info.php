...
<? if ($purinfo=$order->getPurchaseInfo()) { ?>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
      </tr>
      <tr>
        <td><?=join('<br>',$purinfo)?></td>
      </tr>
<? } ?>
      <tr>
...
$return_link = '<a href="' . tep_href_link(FILENAME_RETURN, 'order_id=' . $HTTP_GET_VARS['order_id'] . '&pID=' . ($order->products[$i]['id']), 'NONSSL') . '"><b><u>' . TEXT_RETURN_PRODUCT .'</a></u></b>';
}
...
<?php
    ...
    if (tep_not_null($order->info['ups_track_num'])) {
?>
...
<?php 
$result = tep_db_query("SELECT * FROM orders_shipped WHERE orders_id = '".(int)$_GET['order_id']."'") or die(mysql_error());
echo '<table cellpadding="5" cellspacing="0" border="0">';
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {

echo '<tr><td>' . $row['ship_carrier'] .'</td><td><a style="font:bold 11px arial;" href="'. sprintf(UPS_TRACKING_URL, $row["tracking_number"]).'" target="_new"> ' . $row["tracking_number"] . ' </a></td><td>Shipped: '. date('m/d/Y', strtotime($row["ship_date"])) .'</td></tr>';
}
echo '</table>';
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
          <tr class="infoBoxContents">
            <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><a href="http://trkcnfrm1.smi.usps.com/PTSInternetWeb/InterLabelInquiry.do?origTrackNum=<?php echo $order->info['usps_track_num']; ?>" target="_blank"><?php echo $order->info['usps_track_num']; ?></a>&nbsp;&nbsp;USPS</td>
              </tr>
            </table></td>
          </tr>
        </table>
