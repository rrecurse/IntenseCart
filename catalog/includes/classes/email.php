 ...
 */
    ...
    function attach_cid($url) {
      if (!isset($this->cids)) $this->cids=Array();
      if (isset($this->cids[$url])) return 'cid:'.$this->cids[$url];
      $p=parse_url($url);
      if ($p['host']!=SITE_DOMAIN) return $url;
      $f=DIR_FS_SITE_CATALOG.$p['path'];
      if (!file_exists($f)) return $url;
      $image=$this->get_file($f);
      if ($image===false) return $url;
      $cid=$this->add_html_image($image, basename($f), $this->image_types[preg_replace('/.*\./','',$url)]);
      $this->cids[$url]=$cid;
      return 'cid:'.$cid;
    }
...
// Build the list of image extensions
      ...
      $this->html=preg_replace('/(src=)"([^"]+\.(' . implode('|', $extensions).'))"/Uie','"\\1\\"".$this->attach_cid("\\2")."\\""', $this->html);
    }
 ...
 */
    ...
    function add_html($html, $text = NULL, $images_dir = NULL) {
      $this->html = str_replace(array("\r\n", "\n", "\r", "<br /><br /><br /><br />"), array("<br />","<br />","<br />","<br /><br />"), $html);
