...
/*
    ...
    Set local configuration properties from configuration DB constants
  
    Parameters:
  
      $prefix      sting     Prefix for the DB constants
...
*/
    ...
    if (!isset($this->attributes_array)) {
      ...
      while ($products_options_name = tep_db_fetch_array($products_options_name_query)) {
        $track_stock=(STOCK_CHECK=='true' && $products_options_name['products_options_track_stock']);
        $products_options_array = array();
// Modified by MegaJim - stock tracking
        $products_options_query = tep_db_query("select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix, pov2p.products_id AS values_products_id from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov LEFT JOIN ".TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS." pov2p ON pov.products_options_values_id=pov2p.products_options_values_id where pa.products_id = '" . (int)$this->products_id . "' and pa.options_id = '" . (int)$products_options_name['products_options_id'] . "' and pa.options_values_id = pov.products_options_values_id and pov.language_id = '" . (int)$languages_id . "'");
        while ($products_options = tep_db_fetch_array($products_options_query)) {
	  $optn_text=$products_options['products_options_values_name'];
          if ($products_options['options_values_price'] != '0') {
            $optn_text .= ' (' . $products_options['price_prefix'] . $currencies->display_price($products_options['options_values_price'], tep_get_tax_rate($this->products_tax_class_id)) .')';
          }
	  if (STOCK_CHECK=='true' && $products_options['values_products_id']) {
	    $track_stock=1;
	    if (tep_check_stock($products_options['values_products_id'],1)) $optn_text .= ' -'.TEXT_OUT_OF_STOCK;
	  }
          $products_options_array[] = array('id' => $products_options['products_options_values_id'], 'text' => $optn_text);
        }
        if (isset($cart->contents[$this->products_id]['attributes'][$products_options_name['products_options_id']]))
          ...
          $selected = 0;
        $this->attributes_array[]=array('oid'=>$products_options_name['products_options_id'],
    ...
    foreach($this->attributes_array AS $attr) {
      if (($build_stocked && $attr['track_stock']) || ($build_nonstocked && !$attr['track_stock'])) $attributes[]=$attr;
    }
    return $attributes;
    }
