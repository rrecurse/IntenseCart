<?php
  ...
  class wishlist {
	...
	// merge current wishlist items in database
		...
		$wishlist_session = tep_db_query("select * from " . TABLE_WISHLIST . " where customers_id = '" . $customer_id . "'");
		while($wishlist = tep_db_fetch_array($wishlist_session)) {
			$this->wishID[$wishlist['products_id']] = array($wishlist['products_id'],'wish_box_id'=>$wishlist['wish_box_id']);
		// attributes
    ...
    function get_items($wish_box) {
      $rs=Array();
      if (is_array($this->wishID))
	foreach ($this->wishID AS $pid=>$wish) if ($wish['wish_box_id']==$wish_box) $rs[]=$pid;
      return $rs;
    }

    function move_items($pids,$wish_box) {
      if (!is_array($pids)) $pids=Array($pids);
      tep_db_query("UPDATE ".TABLE_WISHLIST." SET wish_box_id=".(isset($wish_box)?"'".addslashes($wish_box)."'":'NULL')." WHERE products_id IN ('".join("','",$pids)."') AND customers_id='$customer_id'");
      foreach ($pids AS $pid) if (isset($this->wishID[$pid])) $this->wishID[$pid]['wish_box_id']=$wish_box;
    }
