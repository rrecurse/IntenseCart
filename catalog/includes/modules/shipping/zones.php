...
//      print_r($order);
      ...
      $dest_state=tep_db_read("SELECT zone_code FROM zones WHERE zone_name='".addslashes($order->delivery['state'])."'",NULL,'zone_code');
      $dest_zone = 0;
      $error = false;
	...
	$zone='';
        if (in_array(($zone=$dest_country), $country_zones) || in_array('*',$country_zones) || in_array(($zone=$dest_country.'-'.$dest_state), $country_zones)) {
          $dest_zone = $i;
          break;
        }
      }
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0; $i<$size; $i+=2) {
          if (preg_match('/^\$(.*)/',$zones_table[$i],$vp)?($order->info['subtotal']<=$vp[1]):($shipping_weight<=$zones_table[$i])) {
            $shipping = $zones_table[$i+1];
            $shipping_method = ' ' . $zone . ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_ZONES_TEXT_UNITS;
            break;
          }
        }
	...
	$this->quotes=Array();
        if ($shipping >= 0) {
	  if (preg_match('/(.*)\%$/',$shipping,$shp)) $shipping=sprintf("%.2f",$shp[1]*$order->info['subtotal']/100);
	  $handling=constant('MODULE_SHIPPING_ZONES_HANDLING_' . $dest_zone);
          $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_ZONES_TEXT_TITLE,
                            'methods' => array());
			    
	  if (preg_match('/:/',$handling)) {
	    foreach (explode(',',$handling) AS $hdl) {
	      list ($m,$c) = explode(':',$hdl);
		$this->quotes['methods'][]=array('id' => $m,
                	                 'title' => $m.': '.$shipping_method,
                	                 'cost' => ($shipping * $shipping_num_boxes) + $c);
	    }
          $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZONES_HANDLING_' . $dest_zone);
	  } else {
			    $this->quotes['methods'][]=array('id' => $this->code,
                			                 'title' => MODULE_SHIPPING_ZONES_TEXT_WAY . $shipping_method,
                			                 'cost' => $shipping_cost = ($shipping * $shipping_num_boxes) + $handling);
	  }
        }
      }
