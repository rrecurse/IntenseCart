...
// class methods
    ...
    function javascript_validation() {
      $js = '  if (payment_value == "' . $this->code . '") {' . "\n" .
            '    var cc_owner = document.checkout.USAEPAY_cc_name.value;' . "\n" .
            '    var cc_number = document.checkout.USAEPAY_cc_num.value;' . "\n" .
    ...
    function selection() {
      ...
      $selection = array('id' => $this->code,
                         ...
                         'fields' => array(array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_OWNER,
                                                 'field' => tep_draw_input_field('USAEPAY_cc_name', $order->billing['firstname'] . ' ' . $order->billing['lastname'])),
                                           array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_NUMBER,
                                                 'field' => tep_draw_input_field('USAEPAY_cc_num')),
                                           array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_EXPIRES,
                                                 'field' => tep_draw_pull_down_menu('USAEPAY_cc_exp_mon', $expires_month) . '&nbsp;' . tep_draw_pull_down_menu('USAEPAY_cc_exp_year', $expires_year)),
					   array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_CVV2,
                                                 'field' => tep_draw_input_field('USAEPAY_cc_cvv2', '', 'style="width:40px" maxlength="4"'))));
      return $selection;
    }
    ...
    function before_process() {
      ...
      $tran->key=MODULE_PAYMENT_USAEPAY_KEY;
      $tran->testmode=(MODULE_PAYMENT_USAEPAY_TEST=='True');
      $tran->card=$_POST['USAEPAY_cc_num'];		// card number, no dashes, no spaces
      $tran->exp=$_POST['USAEPAY_cc_exp_mon'].$_POST['USAEPAY_cc_exp_year'];			// expiration date 4 digits no /
      ...
      $tran->cardholder=$_POST['USAEPAY_cc_name']; 	// name of card holder
      ...
      $tran->cvv2=$_POST['USAEPAY_cvv2'];			// cvv2 code	
    ...
    function install() {
      if (CORE_PERMISSION) {
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable USAEPAY Module', 'MODULE_PAYMENT_USAEPAY_STATUS', 'True', 'Do you want to accept credit card payments via USAEPAY?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('USAEPAY Test Mode', 'MODULE_PAYMENT_USAEPAY_TEST', 'False', 'Do you want to run USAEPAY in test mode?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
//        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Split Credit Card E-Mail Address', 'MODULE_PAYMENT_USAEPAY_EMAIL', '', 'If an e-mail address is entered, the middle digits of the credit card number will be sent to the e-mail address (the outside digits are stored in the database with the middle digits censored)', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_USAEPAY_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0' , now())");
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment Zone', 'MODULE_PAYMENT_USAEPAY_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Order Status', 'MODULE_PAYMENT_USAEPAY_ORDER_STATUS_ID', '0', 'Set the status of orders made with this payment module to this value', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
        tep_db_query("insert into " . TABLE_CORE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Access Key', 'MODULE_PAYMENT_USAEPAY_KEY', '897asdfjha98ds6f76324hbmnBZc9769374ybndfs876', 'USAEPAY Access Key', '6', '0', now())");
      }
    ...
    function keys() {
      return array('MODULE_PAYMENT_USAEPAY_STATUS', 'MODULE_PAYMENT_USAEPAY_TEST', 'MODULE_PAYMENT_USAEPAY_EMAIL', 'MODULE_PAYMENT_USAEPAY_ZONE', 'MODULE_PAYMENT_USAEPAY_ORDER_STATUS_ID', 'MODULE_PAYMENT_USAEPAY_SORT_ORDER', 'MODULE_PAYMENT_USAEPAY_KEY');
    }
  }
?>
