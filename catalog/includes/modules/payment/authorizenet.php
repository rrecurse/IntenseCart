  ...
  class authorizenet {
      ...
      global $order;
		...
		if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_AUTHORIZENET_ZONE > 0) ) {
	        ...
	        $check_query = tep_db_query("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " 
										 WHERE geo_zone_id = '" . MODULE_PAYMENT_AUTHORIZENET_ZONE . "' 
	...
	function before_process() {
		...
		if($HTTP_POST_VARS['x_response_code'] == '2') {
			...
			error_log(MODULE_PAYMENT_AUTHORIZENET_TEXT_DECLINED_MESSAGE . (!empty($order->orderid) ? "\n\n Order: ". $order->orderid : ""), 1, STORE_OWNER_EMAIL_ADDRESS);
		}
		...
		error_log(MODULE_PAYMENT_AUTHORIZENET_TEXT_ERROR_MESSAGE . (!empty($order->orderid) ? "\n\n Order: ". $order->orderid : ""), 1, STORE_OWNER_EMAIL_ADDRESS);
    }
    ...
    function get_error() {
      ...
      return $error;
	  error_log(MODULE_PAYMENT_AUTHORIZENET_TEXT_ERROR . (!empty($order->orderid) ? "\n\n Order: ". $order->orderid : ""), 1, STORE_OWNER_EMAIL_ADDRESS);
    }
