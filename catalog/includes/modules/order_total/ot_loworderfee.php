...
*/
  ...
  class ot_loworderfee {
    var $title, $output, $sppc_customer_group_id;
	...
	$min=IXdb::read("SELECT customers_extra_value FROM customers_extra WHERE customers_id='$customer_id' AND customers_extra_key='order_minimum'",NULL,'customers_extra_value');
	if (!$min) $min=MODULE_ORDER_TOTAL_LOWORDERFEE_ORDER_UNDER;

        if ( ($pass == true) && ( ($order->info['total'] - $order->info['shipping_cost']) < $min) ) {
	  $fee=max((float) IXdb::read("SELECT customers_extra_value FROM customers_extra WHERE customers_id='$customer_id' AND customers_extra_key='order_surcharge_amount'",NULL,'customers_extra_value'),(float) IXdb::read("SELECT customers_extra_value FROM customers_extra WHERE customers_id='$customer_id' AND customers_extra_key='order_surcharge_percent'",NULL,'customers_extra_value')*$order->info['total']/100);
	  if (!$fee) $fee=MODULE_ORDER_TOTAL_LOWORDERFEE_FEE;
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from IXcore." . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_ORDER_TOTAL_LOWORDERFEE_STATUS'");
    ...
    function install() {
      ...
      tep_db_query("insert into IXcore." . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_LOWORDERFEE_SORT_ORDER', '4', 'Sort order of display.', '6', '2', now())");
      ...
      tep_db_query("insert into IXcore." . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, date_added) values ('Order Fee', 'MODULE_ORDER_TOTAL_LOWORDERFEE_FEE', '5', 'Low order fee.', '6', '5', 'currencies->format', now())");
