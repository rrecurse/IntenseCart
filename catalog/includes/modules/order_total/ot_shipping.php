  ...
  class ot_shipping {
    ...
    function process() {
      ...
      if (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true') {
        ...
        if ( ($pass == true) && !$sppc_customer_group_id && $order->info['shipping_cost']<15 && ( ($order->info['total'] - $order->info['shipping_cost']) >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER) ) {
      ...
      if (1 || tep_not_null($order->info['shipping_method'])) {
	  ...
	  $order->info['shipping_tax_amount'] = tep_calculate_tax($order->info['shipping_cost'], $shipping_tax);

          if (DISPLAY_PRICE_WITH_TAX == 'true') $order->info['shipping_cost'] += tep_calculate_tax($order->info['shipping_cost'], $shipping_tax);
        }

        $this->output[] = array('title' => ($order->info['shipping_method']?$order->info['shipping_method']:'Shipping') . ':',
