...
//      $_SESSION['cot_gv'] = false;     // old code - Strider
	...
	function update_credit_account($i) {
		...
		if(preg_match('/^GIFT/', addslashes($order->products[$i]['model']))) {
			...
			if(MODULE_ORDER_TOTAL_GV_QUEUE == 'false') {
				...
				if ($customer_gv) {
					$gv_update = tep_db_query("UPDATE " . TABLE_COUPON_GV_CUSTOMER . " 
											   SET amount = '" . $total_gv_amount . "' 
											   WHERE customer_id = '" . $customer_id . "'
											  ");
				} else {
					$gv_insert = tep_db_query("INSERT INTO " . TABLE_COUPON_GV_CUSTOMER . " 
				...
				$gv_insert = tep_db_query("INSERT INTO " . TABLE_COUPON_GV_QUEUE . "
										   SET customer_id = '" . $customer_id . "', 
										   order_id = '" . $insert_id . "', 
    ...
    function apply_credit() {
      global $order, $customer_id, $coupon_no, $cc_gv_id;
      $gv_payment_amount=0;
      if (tep_session_is_registered('cc_gv_id')) {
        $balance=tep_db_read("SELECT * FROM coupon_gv_balance WHERE coupon_id='$cc_gv_id'",NULL,'gv_balance');
        $gv_payment_amount = $this->deduction;
        if ($gv_payment_amount>$balance) $gv_payment_amount=$balance;
        tep_db_query("UPDATE coupon_gv_balance SET gv_balance=gv_balance-'$gv_payment_amount' WHERE coupon_id='$cc_gv_id'");
        if ($balance-$gv_payment_amount<0.005) tep_db_query("UPDATE coupons SET coupon_active='N' WHERE coupon_id='$cc_gv_id'");
      }
      return $gv_payment_amount;
    }
	...
	function collect_posts() {
		...
		if($_POST['gv_redeem_code']) {
			...
			$gv_query = tep_db_query("SELECT coupon_id, 
											 coupon_type, 
											 coupon_amount 
									  FROM ". TABLE_COUPONS ." 
...
//error_log($gv_result);
        if (tep_db_num_rows($gv_query) > 0) {
			...
			if(tep_db_num_rows($redeem_query) < 1 && $gv_result['coupon_type'] == 'G') {
		...
		if($total_gv_amount < 0.01) { 
         	tep_db_query("UPDATE " . TABLE_COUPONS . " set coupon_active = 'N' where coupon_id = '" . $gv_result['coupon_id'] . "'");
		}

          $gv_redeem = tep_db_query("INSERT INTO " . TABLE_COUPON_REDEEM_TRACK . " 
									 SET coupon_id = '". $gv_result['coupon_id'] ."', 
          ...
          if ($customer_gv) {
            ...
            $gv_update = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = amount+'" . $total_gv_amount . "' where customer_id = '" . $customer_id . "'");
          } else {
    ...
    function calculate_credit($amount) {
      ...
      if (!tep_session_is_registered('cc_gv_id')) return 0;
      $gv_payment_amount = mysql_result(tep_db_query("SELECT gv_balance FROM coupon_gv_balance WHERE coupon_id='$cc_gv_id'"), 0);
    ...
    function calculate_tax_deduction($amount, $od_amount, $method) {
      ...
      switch ($method) {
		...
		// # in some rare cases we get a divide by zero error with $ratio1
		// # the solution - use $amount in conditon only if $amount is greater then zero
        $ratio1 = ($amount != 0) ? tep_round($gd_amount / $amount, 2) : 0; 
