  ...
  function tep_activate_banners() {
    ...
    if (tep_db_num_rows($banners_query) > 0) {
      while ($banners = tep_db_fetch_array($banners_query)) {
        if (date('Y-m-d H:i:s', time()) >= $banners['date_scheduled']) {
    ...
    if (tep_db_num_rows($banners_query)) {
      while ($banners = tep_db_fetch_array($banners_query)) {
        if (tep_not_null($banners['expires_date'])) {
          if (date('Y-m-d H:i:s', time()) >= $banners['expires_date']) {
...
// # Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    if ($action == 'dynamic') {
      ...
      if ($banners['count'] > 0) {
        $banner = tep_random_select("select banners_id, banners_title, banners_image, banners_html_text, banners_new_window from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . $identifier . "'");
      } else {
      ...
      if (is_array($identifier)) {
        ...
        $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text, banners_new_window from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "'");
    ...
    if (tep_not_null($banner['banners_html_text'])) {
      $banner_string = $banner['banners_html_text'];
		...
		$banner_string = '<a class="banner" href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&amp;goto=' . $banner['banners_id']) . '" title="'.$banner['banners_title'].'" '.($banner['banners_new_window'] ? 'target="_blank"' : '').'" onclick="onGAPromo(\''.$banner['banners_id'].'\',\''.$banner['banners_title'].'\', \''.$banner['banners_image'].'\', \''.$_SERVER['REQUEST_URI'].'\', \'click\');">' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';

		$banner_string .="
<script>
	onGAPromo('".$banner['banners_id']."','".$banner['banners_title']."', '".$banner['banners_image']."', '".$_SERVER['REQUEST_URI']."', 'impression');
</script>";
