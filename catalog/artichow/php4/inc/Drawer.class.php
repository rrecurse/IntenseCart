...
class awDrawer {
	 ...
	 */
	 function string(&$text, $point, $width = NULL) {
		...
		$font->draw($this, $point, $text, $width);
...
class awGradientDrawer {
	 ...
	 */
	 ...
	 function drawFilledFlatTriangle($gradient, $a, $b, $c) {
		...
		if($gradient->angle !== 0) {
			awImage::drawError("Class GradientDrawer: Flat triangles can only be used with 0 degree gradients.");
		}
	
		// Look for right-angled triangle
		if($a->x !== $b->x and $b->x !== $c->x) {
			awImage::drawError("Class GradientDrawer: Not right-angled flat triangles are not supported yet.");
		}
	 ...
	 function filledRectangle($gradient, $p1, $p2) {
		...
		if(is_a($gradient, 'awLinearGradient')) {
			...
			awImage::drawError("Class GradientDrawer: This gradient is not supported by rectangles.");
		}
	 ...
	 function filledPolygon($gradient, &$polygon) {
		...
		if(is_a($gradient, 'awLinearGradient')) {
			...
			awImage::drawError("Class GradientDrawer: This gradient is not supported by polygons.");
		}
	 ...
	 function filledEllipse($gradient, $x1, $y1, $x2, $y2) {
		...
		if(is_a($gradient, 'awRadialGradient')) {
			...
			awImage::drawError("Class GradientDrawer: This gradient is not supported by ellipses.");
		}
	 ...
	 function ellipseRadialGradient($gradient, $x1, $y1, $x2, $y2) {
		...
		if($y1 - $y2 > 0) {
			...
			if($y1 - $y2 != $x2 - $x1) {
				awImage::drawError("Class GradientDrawer: Radial gradients are only implemented on circle, not ellipses.");
			}
	 ...
	 function ellipseLinearGradient($gradient, $x1, $y1, $x2, $y2) {
		...
		if($y1 - $y2 > 0) {
			...
			if($y1 - $y2 != $x2 - $x1) {
				awImage::drawError("Class GradientDrawer: Linear gradients are only implemented on circle, not ellipses.");
			}
