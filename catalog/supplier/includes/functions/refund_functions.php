  ...
  function tep_get_return_reason() {
   ...
   function tep_remove_return($order_id, $restock = false) {
    if ($restock == 'on') {
      ...
      tep_db_query("update " . TABLE_PRODUCTS . " set products_quantity = products_quantity + " . $order['products_quantity'] . ", `last_stock_change` = NOW() where products_id = '" . $order['products_id'] . "'");
