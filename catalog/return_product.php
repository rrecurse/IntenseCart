...
// ############################################
  ...
  $order = new order(isset($_GET['order_id']) ? $_GET['order_id'] : $_GET['oID']);
  ...
  if($_GET['action']) {
    switch ($_GET['action']) {
      ...
      case 'update':
         ...
         $order_returns_query = tep_db_query("SELECT * FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op where o.orders_id = op.orders_id and o.orders_id = '" . $HTTP_GET_VARS['oID'] . " ' and op.products_id = '" . $HTTP_GET_VARS['pID'] . "'");
         $orders_return = tep_db_fetch_array($order_returns_query);

         if ($HTTP_POST_VARS['returns_quantity'] > $orders_return['products_quantity']) {
             tep_redirect(FILENAME_RETURN. '&error=yes&order_id=' . $HTTP_GET_VARS['oID'] . '&pID=' . $HTTP_GET_VARS['pID']);
         }
		 ...
		 $payment_method = mysql_result(mysql_query("SELECT payment_method FROM orders WHERE orders_id='".$order_id."'"),0);
 ...
 // # not present at moment
   $support_error = false;
	...
	$tpl['link'] = array(
	    account_history => tep_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . $order_id, 'SSL', false),
	...
	$admin_email_query = tep_db_query("SELECT support_assign_email, support_assign_name 
									   FROM " . TABLE_SUPPORT_ASSIGN . " 
									   WHERE support_assign_id = '" . $default_email['admin_id'] . "' 
   	...
   	*/
		...
		if(strlen($support_alternate_email) > 0) {
          ...
          // # redirect to confirmation
            tep_redirect(FILENAME_RETURN.'?action=sent&rma_value='. $rma_value . '&return_id=' . $ticket_id);
        } else {
...
} else// if ($HTTP_GET_VARS['action'] == 'new')
{
         ...
         $returned_products_query = tep_db_query("SELECT * FROM " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where o.orders_id = op.orders_id and op.orders_id = '" . $HTTP_GET_VARS['order_id'] . "' and products_id = '" . $HTTP_GET_VARS['pID'] . "'");
         $returned_products = tep_db_fetch_array($returned_products_query);
?>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
      <tr><form name="longsubmit" action="return_product.php?action=insert&oID=<? echo $HTTP_GET_VARS['order_id'] . '&pID=' . $HTTP_GET_VARS['pID']; ?>" method=post>
