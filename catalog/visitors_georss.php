...
//if($_SERVER['HTTP_REFERER'] != '//map.google.com/'.$_SERVER['QUERY_STRING']) header("Location: /");
/////////////////////////////CONFIGURATION/////////////////////////////
//Server info
...
function FlagImage($country_code){
	...
	if (file_exists(DIR_FS_CORE.'admin/'.DIR_WS_IMAGES . 'flags/' . $image)){
		$flag_link = "&lt;img src=&quot;/admin/images/flags/$image &quot; /&gt;";
	}else{
		$flag_link = false;
	}
	return $flag_link;
...
$ip_query = tep_db_query("SELECT * FROM " . TABLE_WHOS_ONLINE . " WHERE latitude != '' AND longitude != '' GROUP BY ip_address");
...
while ($data = mysql_fetch_array($ip_query)){
	...
	$country_name = (!empty($data['country_name'])) ? $data['country_name'] : 'Unknown';
	$region_name = (!empty($data['region_name'])) ? $data['region_name'] : 'Unknown';
	$ip = $data['ip_address'];
	//The accented letters in greek city names are garbled.
	//$city = $data['city'];
	$city = iconv("UTF-8", "ISO-8859-1", (!empty($data['city'])) ? $data['city'] : 'Unknown');
	$active_since = round((time() - $data['time_entry'])/60);
	$flag = FlagImage($country_code);
	$br = "&lt;br /&gt;";
	$customer_id = $data['customer_id'];
	$full_name = $data['full_name'];
	$html = utf8_encode("Country - $country_name $flag $br Region - $region_name $br City - $city $br IP Address - $ip");
	if ($full_name == 'Guest'){
		$g++;
		$name = 'Guest #' . $g;
	} else {
