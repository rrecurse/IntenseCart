...
$OutFile = "feeds/preownedoffice.txt"; //"CHANGEME-full-path-to-file-with-777-dir-and-file-permissions.fr-outfile.txt";
$destination_file = "preownedoffice.txt";  //"CHANGEME-filename-to-upload-to-froogle.txt" ;
$source_file = $OutFile;
$imageURL = 'http://www.preownedoffice.com/images/';
$productURL = 'http://www.preownedoffice.com/product_info.php?products_id=';
$already_sent = array();
...
$ftp_user_name = "preown_admin";
$ftp_user_pass = "pr3own";
...
if($convertCur)
{
$productURL = "http://www.preownedoffice.com/product_info.php?currency=" . $curType . "&products_id=";  //where CURTYPE is your currency type (eg. USD, EUR, GBP)
}
...
$brand = 0;
$upc = 0;   //Not supported by default
$manufacturer_id = 0;  //Not supported by default
$product_type = 0;
$currency = 0;
...
$ship_to = 0;
	$default_ship_to = "ALL"; //this is not binary, not supported by default for individual products.
$ship_from = 0;
	$default_ship_from = "USD"; //this is not binary, not supported by default for individual products.
...
while( $row = mysql_fetch_object( $result ) )
{
	...
	if( $row->prodStatus == 1 || ($optional_sec == 1 && $instock == 1) )
	{
		...
		$output .= tep_href_link('product_info.php', 'products_id=' . $row->id) . "\t" .
