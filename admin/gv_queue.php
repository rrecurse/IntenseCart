...
<?php
		...
		$gv_query_raw = "SELECT c.customers_firstname, 
						 ...
						 LEFT JOIN " . TABLE_COUPON_GV_QUEUE . " gv ON gv.customer_id = c.customers_id
						 WHERE gv.release_flag = 'N'
						 ";

		$gv_query_raw = str_replace(array("\r\n", "\r", "\n"), "", $gv_query_raw);
	    ...
	    if((is_object($gInfo)) && ($gv_list['unique_id'] == $gInfo->unique_id) ) {
    	  echo '<tr class="dataTableRowSelected" onclick="document.location.href=\'' . tep_href_link('gv_queue.php', tep_get_all_get_params(array('gid', 'action')) . 'gid=' . $gInfo->unique_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '              <tr class="'.($ct++&1 ? 'tabEven' : 'tabOdd').' dataTableRow" onclick="document.location.href=\'' . tep_href_link('gv_queue.php', tep_get_all_get_params(array('gid', 'action')) . 'gid=' . $gv_list['unique_id']) . '\'">' . "\n";
    }
?>
...
<?php
  ...
  switch ($_GET['action']) {
    ...
    default:
      ...
      $contents[] = array('align' => 'center','text' => '<a href="' . tep_href_link('gv_queue.php','action=release&gid=' . $gInfo->unique_id,'NONSSL'). '">' . tep_image_button('button_release.gif', IMAGE_RELEASE) . '</a>' . '&nbsp;&nbsp;<a href="' . tep_href_link('gv_queue.php','action=delete&gid=' . $gInfo->unique_id,'NONSSL'). '">' . tep_image_button('button_delete_now.gif', 'Immediate Deletion') . '</a>');
      break;
   }
