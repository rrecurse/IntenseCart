...
// # ********** PAY PER CLICK CONFIGURATION SECTION EOF ************
	...
	$date_from = (!empty($_GET['date_from']) ? date('m/d/Y', strtotime($_GET['date_from'])) : date('m/d/Y',strtotime('-2 months')));
	$date_to = (!empty($_GET['date_to']) ? date('m/d/Y', strtotime($_GET['date_to'])) : date('m/d/Y', time()) );

	$report = (!empty($_GET["report"]) ? $_GET["report"] : 'prod_coverage');
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td style="font:bold 17px arial; white-space:nowrap">&nbsp;Visitors Statistics</td>
            ...
            <td align="right" style="padding:4px 10px 6px 0;">
...
<input type="text" name="date_from" style="font:bold 9px arial;" onclick="self.popUpCalendar(this,this,'mm/dd/yyyy',document);" value="<?php echo $date_from?>" size="12" maxlength="11" textfield></td>
							...
							<td><img src="images/calander2.gif" width="16" height="16" hspace="3" border="0" onclick="self.popUpCalendar(document.report_select.date_from,document.report_select.date_from,'mm/dd/yyyy',document);" style="cursor:pointer"></td>
                        				<td align="center" style="padding-top:1px; padding-left:3px; padding-right:3px;"> - </td>

                        				<td align="right" style="padding-top:2px;"><input type="text" name="date_to" onclick="self.popUpCalendar(document.report_select.date_from,this,'mm/dd/yyyy',document);" style="width:85px; font:bold 9px arial;" value="<?php echo $date_to ?>" size="12" maxlength="11" textfield></td>

                        				<td><img src="images/calander2.gif" width="16" height="16" hspace="3" border="0" onclick="self.popUpCalendar(document.report_select.date_from,document.report_select.date_to,'mm/dd/yyyy',document);" style="cursor:pointer"></td>
...
<?php
	...
	if(isset($_GET["report"])) {
		$item = 'supertracker.php?report='.$_GET["report"];
	} elseif(isset($_GET["special"])) {
		$item = 'supertracker.php?special='.$_GET["special"];
	}

?>
<div style="position:relative; top:0; left:0; height:22px; width:200px;">

<select id="supertrackerSelection" name="report_selector" style="width:196px;" onchange="page_redirect(this,this.form.date_from,this.form.date_to);">
							
<option value=""><?php echo TABLE_TEXT_MENU_TEXTE; ?></option>
<option <?php echo ($item == "supertracker.php?report=refer" ? 'selected' : ''); ?> value="supertracker.php?report=refer"><?php echo TEXT_TOP_REFERRERS; ?></option>
<option <?php echo ($item == "supertracker.php?report=success_refer" ? 'selected' : ''); ?>  value="supertracker.php?report=success_refer"><?php echo TEXT_TOP_SALES;?></option>
<option <?php echo ($item == "supertracker.php?special=geo" ? 'selected' : ''); ?> value="supertracker.php?special=geo"><?php echo TEXT_VISITORS;?></option>								
<option <?php echo ($item == "supertracker.php?special=keywords" ? 'selected' : ''); ?> value="supertracker.php?special=keywords"><?php //echo TEXT_SEARCH_KEYWORDS;?>Search Keywords Used</option>
	
<option <?php echo ($item == "supertracker.php?report=exit" ? 'selected' : ''); ?> value="supertracker.php?report=exit"><?php echo TEXT_TOP_EXIT_PAGES;?></option>
<option <?php echo ($item == "supertracker.php?report=exit_added" ? 'selected' : ''); ?> value="supertracker.php?report=exit_added">Top Exit Pages (Dropped Cart!)</option>
<option <?php echo ($item == "supertracker.php?report=ave_clicks") ? 'selected' : ''; ?> value="supertracker.php?report=ave_clicks"><?php echo TEXT_AVERAGE_CLICKS;?></option>							<option value="supertracker.php?report=ave_time"><?php echo TEXT_AVERAGE_TIME_SPENT;?></option>
<option <?php echo ($item == "supertracker.php?special=prod_coverage" ? 'selected' : ''); ?> value="supertracker.php?special=prod_coverage"><?php echo TEXT_PRODUCTS_VIEWED_REPORT;?></option>
<option <?php echo ($item == "supertracker.php?special=last_ten" ? 'selected' : ''); ?> value="supertracker.php?special=last_ten"><?php echo TEXT_LAST_TEN_VISITORS;?></option>
...
<option <?php echo ($item == "supertracker.php?special=keywords_last72" ? 'selected' : ''); ?> value="supertracker.php?special=keywords_last72"><?php echo TEXT_SEARCH_KEYWORDS_3;?></option>			
<option <?php echo ($item == "supertracker.php?special=keywords_lastweek" ? 'selected' : ''); ?> value="supertracker.php?special=keywords_lastweek"><?php echo TEXT_SEARCH_KEYWORDS_7;?></option>			
<option <?php echo ($item == "supertracker.php?special=keywords_lastmonth" ? 'selected' : ''); ?> value="supertracker.php?special=keywords_lastmonth"><?php echo TEXT_SEARCH_KEYWORDS_30;?></option-->
		  ...
		  <td style="padding-right:10px;" width="20">&nbsp;<a href="javascript:page_redirect(document.report_select.report_selector,document.report_select.date_from,document.report_select.date_to);"><font style="font:bold 11px arial; background-color:#6295FD; color:#FFFFFF;">&nbsp;GO&nbsp;</font></a>                          </td>
		</tr>
</table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td valign="top" style="padding:10px 0 0 10px;" colspan="2">
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
  		<tr>
		    <td>
...
<?php
  ...
  function fmt_date($date) {
	...
	if(isset($_GET['date_from']) && $date_from == fmt_date($_GET['date_from'])) {
		$date_range_query .= " AND time_arrived >= '".date('Y-m-d 00:00:01',strtotime($date_from))."'";
	}

	if(isset($_GET['date_to']) && $date_to == fmt_date($_GET['date_to'])) {
		$date_range_query .= "AND time_arrived < '".date('Y-m-d 23:59:59',strtotime($date_from))."'";
	}

//var_dump($date_range_query);
...
<?php 
	...
	foreach($headings as $header) {
		echo '<td class="dataTableHeadingContent">' . $header . '</td>';
	}
?>
              </tr>
...
<?php
	...
	while($tracker = tep_db_fetch_array($tracker_query)) {
		...
		echo '<tr class="dataTableRow">
				<td class="dataTableContent">'.$counter.'</td>';
		...
		foreach ($row_data as $r) {
			...
			if(strlen($tracker[$r]) > 50) {
				$tracker[$r] = substr($tracker[$r],0,50);
			}
...
?>
     <table width="100%" border="0" cellspacing="0" cellpadding="0">
           ...
           <form name="filter_select" action="supertracker.php" method="get">
					 <input type="hidden" name="special" value="prod_coverage">
             
             <?php echo TEXT_STRING_FILTER;?> 
...
<?php
	  ...
	  $cov_q = tep_db_query("SELECT products_viewed 
							 FROM supertracker 
							 WHERE time_arrived >= '". date('Y-m-d 00:00:01', strtotime($date_from)) ."'
							 AND time_arrived <= '". date('Y-m-d 23:59:59', strtotime($date_to)) ."'
							");
	  ...
	  while ($cov_row=IXdb::fetch($cov_q)) {
	   ...
	   $prod_result = tep_db_query("SELECT p.products_id, pd.products_name 
									FROM ". TABLE_PRODUCTS ." p 
									LEFT JOIN ". TABLE_PRODUCTS_DESCRIPTION ." pd ON pd.products_id = p.products_id 
								   ");
//var_dump($match_agent_string);

		 $prod_coverage = array();
		 while ($prod_row = tep_db_fetch_array($prod_result)) {

			if(isset($view_count[$prod_row['products_id']])) {
				$prod_coverage[$prod_row['products_name']] = $view_count[$prod_row['products_id']];
			}
...
?>
     <table cellpadding="2" cellspacing="0" border="0" width="100%">
		...
		<tr>
			<td class="pageHeading" colspan="2" align="left"><?php echo TABLE_TEXT_PRODUCT_COVERAGE_REPORT; ?></td>
		</tr>		 
		<tr class="dataTableHeadingRow">
			<td class="dataTableHeadingContent"><?php echo TABLE_TEXT_PRODUCT_NAME; ?></td>
			<td class="dataTableHeadingContent"><?php echo TABLE_TEXT_NUMBER_OF_VIEWING; ?></td>
		</tr>			 
<?php
		...
		foreach ($prod_coverage as $prod_name => $hits) {
			...
			echo '<tr>
					<td class="dataTableContent">' . $prod_name . '</td><td class="dataTableContent">' . $hits . '</td>
				</tr>';
		 }		 
?>
		 </table>
