...
Spry.Widget.Accordion.prototype.init = function(element)
{
	...
	this.pendingClass = "AccordionPanelPending";
...
Spry.Widget.Accordion.prototype.openPanel = function(panel)
{
	...
	if (this.pendingPanel && this.pendingPanel!=panelB) {
	  this.removeClassName(this.pendingPanel, this.pendingClass);
	  this.addClassName(this.pendingPanel, this.closedClass);
	...
	if( panelB) {
		...
		if (hook && !eval(hook)) {
	          ...
	          this.addClassName(panelB, this.pendingClass);
		  this.pendingPanel = panelB;
		  return;
		}
	    }
	}

	if (this.pendingPanel == panelB) this.pendingPanel=null;
	...
	{
	        this.removeClassName(panelB, this.closedClass);
	        this.removeClassName(panelB, this.pendingClass);
...
//	while (div && div.nodeType!=1) div=div.nextSibling;
	panel.style.height=(tab.offsetHeight+cont.offsetHeight)+"px";
}
...
{
	if (!Spry.Widget.Accordion.onloadDidFire) return;
	var content = this.getPanelContent(panel);
	var hook = content.getAttribute('onExpanderOpenPanel');
	if (isDefault && !hook)
	{
...
//		this.panelHeight = content.offsetHeight;
	...
	if (isDefault && hook) this.openPanel(panel);
};
...
Spry.Widget.Accordion.PanelAnimator = function(accordion, panel, panelHeight, opts)
{
	...
	for (var i = 0; i < panels.length; i++)
	{
		...
		var t = accordion.getPanelTab(p);
		if (c)
		{
			var h = p.offsetHeight;
			var h0 = t.offsetHeight;
			if (p == panel || h > h0)
			{
				var obj = new Object;
				obj.panel = p;
				obj.content = c;
				obj.overflow = p.style.overflow;
...
//					c.style.height = "0px";
//					c.style.height = "1px";
					if (!this.panelHeight && div) this.panelHeight=div.offsetHeight;
				}

				obj.fromHeight = h;
				obj.toHeight = (p == panel) ? h0+this.panelHeight : h0;
	...
	if (this.timer)
	{
		...
		if (this.stepCount < this.steps)
		{
			for (i = 0; i < this.panelData.length; i++)
			{
				...
				obj.panel.style.overflow = obj.overflow;
			}
		}
	}
...
Spry.Widget.Accordion.PanelAnimator.prototype.animate = function()
{
	...
	if (this.stepCount >= this.steps)
	{
		for (i = 0; i < this.panelData.length; i++)
		{
			...
			obj.panel.style.overflow = obj.overflow;
			obj.panel.style.height = obj.toHeight + "px";
		}
	}
	else
	{
		for (i = 0; i < this.panelData.length; i++)
		{
			...
			obj.panel.style.height = obj.fromHeight + "px";
		}
	}
	if (window.contentChanged) window.contentChanged();
};
