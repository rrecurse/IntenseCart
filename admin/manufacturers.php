<?php
  ...
  if (tep_not_null($action)) {
	...
	$rows=Array();
	foreach (preg_split('/\r?\n/',$_POST['models']) AS $idx=>$mdl) if (trim($mdl)!='') $rows[]="('$manufacturers_id','".addslashes(trim($mdl))."','$idx')";
	tep_db_query("DELETE FROM manufacturers_makes WHERE manufacturers_id='$manufacturers_id'");
	if ($rows) tep_db_query("INSERT INTO manufacturers_makes (manufacturers_id,products_make,sort_order) VALUES ".join(',',$rows));

        if ($manufacturers_image = new upload('manufacturers_image', DIR_FS_CATALOG_IMAGES, '664', array('.jpg', '.gif', '.png'))) {
...
<body style="margin:0 0 0 5px; background-color:transparent;" onload="SetFocus();">
...
<?php
  ...
  switch ($action) {
    ...
    case 'edit':
      ...
      $contents[] = array('text' => '<br><div align=center>' . tep_info_image($mInfo->manufacturers_image, $mInfo->manufacturers_name,SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT).'</div><br>' . TEXT_MANUFACTURERS_IMAGE . '<br>' . tep_draw_file_field('manufacturers_image') . '<br>' . $mInfo->manufacturers_image);
      ...
      $models=Array();
      $mdl_query=tep_db_query("SELECT * FROM manufacturers_makes WHERE manufacturers_id='".$mInfo->manufacturers_id."' ORDER BY sort_order");
      while ($mdl_row=tep_db_fetch_array($mdl_query)) $models[]=$mdl_row['products_make'];

      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_URL . $manufacturer_inputs_string);
      $contents[] = array('text' => '<br> Child Entries <br><textarea name="models" cols="35" rows="5">'.htmlspecialchars(join("\n",$models)).'</textarea>');
    ...
    default:
      if (isset($mInfo) && is_object($mInfo)) {
        ...
        $contents[] = array('text' => '<br><div align=center>' . tep_info_image($mInfo->manufacturers_image, $mInfo->manufacturers_name,SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT).'</div>');
  ...
  if ( (tep_not_null($heading)) && (tep_not_null($contents)) ) {
    echo '            <td width="25%" valign="top" style="padding:0 5px 0 5px;">' . "\n";
