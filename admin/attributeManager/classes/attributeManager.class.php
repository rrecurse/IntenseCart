...
/*abstract*/ class attributeManager /*implements attributeManagerInterface*/ {
	...
	function getAllOptions() {
		if(0 === count($this->arrAllOptions)) {
			$query = amDB::query("select * from ".TABLE_PRODUCTS_OPTIONS." where language_id='".amDB::input($this->intLanguageId)."'");
			while($res = amDB::fetchArray($query)) {
				$this->arrAllOptions[$res['products_options_id']] = $res['products_options_name'];
				$this->arrAllOptionsTrackStock[$res['products_options_id']] = $res['products_options_track_stock']; // Trial Rigadin QTPro
			}
		}
	 ...
	 * @access protected
	 * @author Sam West aka Nimmit - osc@kangaroopartners.com
	 * @return array All options and optoin values
	 */
		...
		if(0 === count($this->arrAllOptionsAndValues)){
			...
			while($res = amDB::fetchArray($query)) {
				if($res['products_options_id'] != $optionsId) {
					...
					$this->arrAllOptionsAndValues[$optionsId]['track_stock'] = $this->arrAllOptionsTrackStock[$optionsId]; // Trial Rigadin QTPro
				}
	...
	function addOption($get) {
		...
		$this->getAndPrepare('track_stock', $get, $track_stock);
		...
		foreach($arrOptions as $option) {
			...
			$arrData = array (
				...
				'products_options_track_stock' => amDB::input($track_stock), // QTPro
	...
	function buildAllTemplatesDropDown() {
		...
		$returnArray = array(array('id' => '0', 'text' => '-- Templates --'));
	...
	function loadTemplate($get) {
		...
		// otherwise nothing in the template exists anymore so just restore the original values, no halm done
		else
