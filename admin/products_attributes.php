...
//++++ QT Pro: End Changed Code
      ...
      case 'add_product_option_values':
        ...
        $to_products_id = tep_db_prepare_input($HTTP_POST_VARS['to_products_id']);
	...
	if ($to_products_id) {
          tep_db_query("insert into " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS . " (products_options_values_id,products_id) VALUES ('" . (int)$value_id . "','$to_products_id')" );
	}
...
//++++ QT Pro: End Changed Code
	...
	if ($to_products_id) {
          tep_db_query("replace into " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS . " (products_options_values_id,products_id) VALUES ('" . (int)$value_id . "','$to_products_id')" );
	} else {
          tep_db_query("delete from " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS . " WHERE products_options_values_id='" . (int)$value_id . "'");
	}
...
<?php
    $per_page = MAX_ROW_LISTS_OPTIONS;
    $values = "select pov.products_options_values_id, pov.products_options_values_name, pov2p.products_id, pov2po.products_options_id, pd.products_name from " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov left join " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " pov2po on pov.products_options_values_id = pov2po.products_options_values_id LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS . " pov2p ON pov.products_options_values_id=pov2p.products_options_values_id LEFT JOIN ".TABLE_PRODUCTS_DESCRIPTION." pd ON pov2p.products_id=pd.products_id AND pd.language_id='$languages_id' where pov.language_id = '" . (int)$languages_id . "' order by pov.products_options_values_id";
    if (!isset($value_page)) {
      $value_page = 1;
    }
...
// By MegaJim - product listing for binding
    $products_lst_query=tep_db_query("SELECT products_id,products_name FROM products_description WHERE language_id='$languages_id' ORDER BY products_name");
    $products_pull_down=Array(Array(id=>'',text=>'--none--'));
    while ($products_lst=tep_db_fetch_array($products_lst_query)) {
      $products_pull_down[]=Array(id=>$products_lst[products_id],text=>$products_lst[products_id].': '.$products_lst[products_name]);
    }
...
?>
              ...
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent">&nbsp;<?php echo TABLE_HEADING_OPT_NAME; ?>&nbsp;</td>
                <td class="dataTableHeadingContent">&nbsp;<?php echo TABLE_HEADING_OPT_VALUE; ?>&nbsp;</td>
                <td class="dataTableHeadingContent">&nbsp;<?php echo TABLE_HEADING_OPT_VALUE_PRODUCT; ?>&nbsp;</td>
                <td class="dataTableHeadingContent" align="center">&nbsp;<?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
              <tr>
...
<?php
    ...
    while ($values_values = tep_db_fetch_array($values)) {
      ...
      $values_products_id = $values_values['products_id']?'<acronym title="'.tep_output_string($values_values['products_name']).'">'.$values_values['products_id'].'</acronym>':'-';
      $rows++;
?>
              <tr class="<?php echo (floor($rows/2) == ($rows/2) ? 'attributes-even' : 'attributes-odd'); ?>">
<?php
	...
	$inputs.="<hr>\nBind to Product:<br>";
	$inputs.=tep_draw_pull_down_menu('to_products_id',$products_pull_down,$values_values['products_id']);
...
?>
                ...
                <td class="smallText"><?=$values_products_id?>&nbsp;</td>
...
<?php
      ...
      $inputs.="<hr>\nBind to Product:<br>";
      $inputs.=tep_draw_pull_down_menu('to_products_id',$products_pull_down);
?>
                </select>&nbsp;</td>
                <td class="smallText" colspan="2"><input type="hidden" name="value_id" value="<?php echo $next_id; ?>"><?php echo $inputs; ?></td>
