<?php
// # implemented an exit() function to stop page from being used - remove exit() to make script usable.
exit();
...
//    $current_path = DIR_FS_DOCUMENT_ROOT;
  ...
  if (strstr($current_path, '..')) $current_path = $editor_root;

  if (!is_dir($current_path)) $current_path = $editor_root;

  if (!preg_match('#'.$editor_root.'#i', $current_path)) $current_path = $editor_root;
      ...
      case 'processuploads':
        for ($i=1; $i<6; $i++) {
          if (isset($GLOBALS['file_' . $i]) && tep_not_null($GLOBALS['file_' . $i])) {
            new upload('file_' . $i, $current_path, '664', array('.jpg', '.gif', '.png', '.swf', '.html', '.css', '.js'));
          }
        }
  ...
  $in_directory = substr(substr($editor_root, strrpos($editor_root, '/')), 1);
  $current_path_array = explode('/', $current_path);
  $document_root_array = explode('/', $editor_root);
  $goto_array = array(array('id' => $editor_root, 'text' => $in_directory));
...
?>
      ...
      <tr><?php echo tep_draw_form('new_file', FILENAME_FILE_MANAGER, 'action=save'); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main"><?php echo TEXT_FILE_NAME; ?> <?php echo $filename_input_field; ?><br></td>
            <td class="main"><?php echo TEXT_FILE_CONTENTS; ?><br>
          </tr>
          <tr>
<!--?php echo tep_draw_area_field('file_contents','776','400',$file_contents, (($file_writeable) ? '' : 'readonly')); ?></td-->
            <td class="main" colspan="2"><?php echo tep_draw_textarea_field('file_contents', 'soft', '80', '20', $file_contents, (($file_writeable) ? '' : 'readonly')); ?></td>
...
<?php
    ...
    while ($file = $dir->read()) {
      if ( ($file != '.') && ($file != 'CVS') && ( ($file != '..') || ($current_path != $editor_root) ) ) {
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent" colspan="4">File name</td>
                <td class="dataTableHeadingContent">File size</td>
               <!--/* //<td class="dataTableHeadingContent" align="center"><?php echo TABLE_HEADING_PERMISSIONS; ?></td>
                ...
                <td class="dataTableHeadingContent">Modified</td>
...
?>
                <td class="dataTableContent" onclick="document.location.href='<?php echo tep_href_link(FILENAME_FILE_MANAGER, $onclick_link); ?>'" colspan="4"><?php echo '<a href="' . $link . '">' . $icon . '</a>&nbsp;' . $contents[$i]['name']; ?></td>
