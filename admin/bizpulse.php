<?php
  ...
  require(DIR_FS_CATALOG.'artichow/ScatterPlot.class.php');
	...
	$width = (isset($_GET['width'])) ? $_GET['width'] : 670;
	...
	$total_query = tep_db_query("SELECT YEAR(o.date_purchased) AS y,
								 MONTH(o.date_purchased) AS m,
								 SUM(COALESCE(ot.value, 0.00)) AS s
								 FROM ".TABLE_ORDERS." o
								 LEFT JOIN ".TABLE_ORDERS_TOTAL." ot ON o.orders_id = ot.orders_id AND ot.class= 'ot_total'
								 WHERE o.date_purchased BETWEEN '".date('Y-01-01 00:00:01', strtotime('-1 years'))."' AND NOW()
	...
	$qty_total_query = tep_db_query("SELECT YEAR(o.date_purchased) AS y,
									 MONTH(o.date_purchased) AS m,
	...
	while ($qty_total = tep_db_fetch_array($qty_total_query)) {

		if(!isset($total[$qty_total['y']])) { 
			$total[$qty_total['y']] = array();
		}

		$total[$qty_total['y']][$qty_total['m']] = array('qty' => $qty_total['q']);
	}
		
	$ytd_qty = (isset($total[date('Y')])) ? $total[date('Y')] : array();
	$prv_qty = (isset($total[date('Y')-1])) ? $total[date('Y')-1] : array();

	$returns_query = tep_db_query("SELECT YEAR(o.date_purchased) AS y,
								   MONTH(o.date_purchased) AS m,
								   SUM(rpd.products_quantity) as r
								   FROM ".TABLE_ORDERS." o
								   LEFT JOIN ".TABLE_RETURNS_PRODUCTS_DATA." rpd ON rpd.order_id = o.orders_id
								   LEFT JOIN ".TABLE_RETURNS." rp ON rp.order_id = rpd.order_id
								   WHERE o.date_purchased BETWEEN '".date('Y-01-01 00:00:01', strtotime('-1 years'))."' AND NOW()
								   AND o.orders_status > 0
	...
	while($returns = tep_db_fetch_array($returns_query)) {
		...
		if(!isset($returned[$returns['y']])) { 
			$returned[$returns['y']] = array();
		}

		$returned[$returns['y']][$returns['m']]['ret'] = $returns['r'];
	}

	$ytd_ret = (isset($returned[date('Y')])) ? $returned[date('Y')] : array();
	$prv_ret = (isset($returned[date('Y')-1])) ? $returned[date('Y')-1] : array();

	$canc_query = tep_db_query("SELECT YEAR(o.date_purchased) AS y,
								MONTH(o.date_purchased) AS m,
								SUM(COALESCE(ot.value, 0.00)) AS s,
								SUM(op.products_quantity) AS q
								FROM ".TABLE_ORDERS." o
	...
	$qty_max = max(1, $max_val_qty, $max_val_qtyprv);
