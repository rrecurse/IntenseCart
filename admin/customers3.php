...
class edit_customers extends IXEditList {
  function getListQuery() {
    return "select * from stores s LEFT JOIN address_book ab ON (s.address_book_id=ab.address_book_id) ORDER BY ab.entry_postcode";
  }
...
<tr><td><?=$item['stores_title']?></td><td align="right"><?=$item['entry_postcode']?> <?=$item['zone_code']?> <?=$item['countries_iso_code_2']?></td><td width="50">[<a href="<?=$this->makeLink('edit='.$item['stores_id'])?>">Edit</a>]</td><td width="50">[<a href="<?=$this->makeLink('delete='.$item['stores_id'])?>" onClick="return window.confirm('Do you want to delete this entry?')">Delete</a>]</td></tr>
</table>
<?
  }
