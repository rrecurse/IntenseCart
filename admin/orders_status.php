<?php
  ...
  $definedStatus=Array(0=>'Canceled',1=>'Pending',2=>'Processed',3=>'Complete');
...
<? if ($messageStack->size > 0) { ?>
      <tr><td><?=$messageStack->output()?></td></tr>
<? } ?>
      <tr>
...
//    } else {
      echo '                <td class="dataTableContent"><b>' . $orders_status['orders_status_name'].'</b>' .(isset($definedStatus[$orders_status['orders_status_id']])?' ('.$definedStatus[$orders_status['orders_status_id']].')':''). '</td>' . "\n";
//    }
?>
...
//      $contents[] = array('text' => '<br>' . tep_draw_checkbox_field('cancel_order') . ' Cancel order and restore inventory?');
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ORDERS_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->orders_status_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>'.(isset($definedStatus[$oInfo->orders_status_id])?'':'<a href="' . tep_href_link(FILENAME_ORDERS_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->orders_status_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>'));
