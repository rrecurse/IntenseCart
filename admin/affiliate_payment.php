...
// ############################################
	...
	$payments_status_query = tep_db_query("SELECT affiliate_payment_status_id, affiliate_payment_status_name 
										   FROM " . TABLE_AFFILIATE_PAYMENT_STATUS . " 
										   WHERE affiliate_language_id = '" . $languages_id . "'
										 ");
	...
	switch ($_GET['action']) {
		...
		case 'start_billing':
			...
			$affiliate_payment_query = tep_db_query("SELECT a.affiliate_id, sum(a.affiliate_payment) 
													 ...
													 GROUP by a.affiliate_id HAVING SUM(a.affiliate_payment) >= '" . AFFILIATE_THRESHOLD . "'
													");
			...
			while ($affiliate_payment = tep_db_fetch_array($affiliate_payment_query)) {
				...
				$affiliate_orders_query=tep_db_query("SELECT a.affiliate_orders_id
													  FROM " . TABLE_AFFILIATE_SALES . " a
													  LEFT JOIN " . TABLE_ORDERS . " o ON a.affiliate_orders_id = o.orders_id
											          WHERE a.affiliate_billing_status != 1 
													  AND o.orders_status >= " . AFFILIATE_PAYMENT_ORDER_MIN_STATUS . " 
				...
				tep_db_query("UPDATE " . TABLE_AFFILIATE_SALES . " 
		        ...
		        $affiliate_billing_query = tep_db_query("SELECT SUM(affiliate_payment) AS affiliate_payment
