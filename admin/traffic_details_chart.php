...
$srclst=isset($_GET['traffic'])?$_GET['traffic']:Array('');
foreach ($srclst AS $idx=>$srcl) {
  $conds=Array();
  foreach (Array('traffic_type','traffic_source','traffic_keywords') AS $fld) if (isset($_GET[$fld][$idx])) $conds[]="$fld='".$_GET[$fld][$idx]."'";
  $trk_cond=$conds?join(' AND ',$conds):'1';
...
$graph->border->hide();
$grp->setBackgroundColor(make_color('bg_color','F0F5FB'));
