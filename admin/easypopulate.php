...
$froogle_product_info_path = "http://" . HTTP_SERVER . "/product_info.php";

// **** Froogle product image path ****
// Set this to the path to your images directory
global $froogle_image_path;
$froogle_image_path = "http://" . HTTP_SERVER . "/catalog/images/";
...
$default_these = array(
	...
	'v_products_image_xl_7',
	'v_products_image_xl_8',
	'v_categories_id',
	'v_products_price',
	...
	while ($row){
		...
		foreach ($langcode as $key => $lang){
			...
			if(isset($filelayout['v_products_head_title_tag_' . $lid])){
				...
				$row['v_products_info_alt_' . $lid] 	= $row2['products_info_alt'];
			}
		}
...
header("Pragma: ");
    ...
    if (!$fp = fopen($tmpfname, "w+")) {
         print "Cannot open file ($tmpfname)";
         exit;
    }

    // Write $somecontent to our opened file.
    if (!fwrite($fp, $filestring)) {
        print "Cannot write to file ($tmpfname)";
        exit;
    }
...
function ep_create_filelayout($dltype){
	...
	case 'full':
		...
		$filelayout = array(
			...
			'v_products_image_xl_7' => $iii++,
			'v_products_image_xl_8' => $iii++,
			);
...
//// attributes stock add end 		
    ...
    // VJ product attribs end
		...
		$filelayout_sql = "SELECT
			...
			p.products_image_xl_7 as v_products_image_xl_7,
			p.products_image_xl_8 as v_products_image_xl_8,
...
function walk( $item1 ) {
	...
	$item1 = trim($item1, "\0x20,\n,\r,\0"); // Rigadin: use trim instead of ltrim/rtrim and do not trim tabulators
...
//				";
			...
			// end support for Header Controller 2.0
			// support for products info extra fields
			if(isset($filelayout['v_products_info_' . $lang['id'] ])){
				$row['v_products_info_' . $lang['id']] 	= $row2['products_info'];
				$row['v_products_info_alt_' . $lang['id']] 	= $row2['products_info_alt'];
			}
		}
	...
	foreach ($langcode as $lang){
		...
		if (isset($filelayout['v_products_name_' . $l_id ])){
			...
			// end support for Products extra info
			if(isset($filelayout['v_products_head_title_tag_' . $l_id])){
				$v_products_info[$l_id] 	= $items[$filelayout['v_products_info_' . $l_id]];
				$v_products_info_alt[$l_id] 	= $items[$filelayout['v_products_info_alt_' . $l_id]];
			}
		}
	}
	...
	if ($v_products_model != "") {
		...
		if (tep_db_num_rows($result) == 0)  {
			...
			$query = "UPDATE "'. TABLE_PRODUCTS ."
					  SET	products_price = '".$v_products_price."',
					  products_qty_blocks = '".$v_products_qty_blocks ."',
					  ...
					  products_image_xl_7 = '". $v_products_image_xl_7 ."',
					  products_image_xl_8 = '". $v_products_image_xl_8 ."',
						...
						if (isset($v_products_head_title_tag)){
							...
							$sql =
								"UPDATE ".TABLE_PRODUCTS_DESCRIPTION." SET
									...
									products_info_alt = '".$v_products_info_alt[$key] . "',
									products_url = '" . $v_products_url[$key] ."',
