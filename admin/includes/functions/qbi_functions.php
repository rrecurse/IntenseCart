...
function log_open($logtype) {
  $loghandle=fopen(DIR_FS_SHARE."qbi_output/error_log_".$logtype.".txt", "wb");
  return $loghandle;
}
...
function ship_substitute($ordersid) {
  ...
  if ($ship_method=tep_db_fetch_array($ship_method_query)) {
    ...
    $ship_match_query=tep_db_query("SELECT * FROM ".TABLE_QBI_SHIPOSC." AS so, ".TABLE_QBI_SHIPOSC_SHIPQB." AS soqb, ".TABLE_QBI_SHIPQB." AS sqb WHERE qbi_shiposc_car_text='".addslashes($carrier)."' AND qbi_shiposc_serv_text='".addslashes($service)."' AND so.qbi_shiposc_car_code=soqb.qbi_shiposc_car_code AND so.qbi_shiposc_serv_code=soqb.qbi_shiposc_serv_code AND sqb.qbi_shipqb_refnum=soqb.qbi_shipqb_refnum");
