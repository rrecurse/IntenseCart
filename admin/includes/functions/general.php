...
// # Redirect to another page or site
  function tep_redirect($url) {
	...
	function tep_parse_input_field_data($data, $parse) {
		...
		if(is_array($data)) { 
			foreach($data as $d) $result = strtr(trim($d), $parse);
		} else {
			$result = strtr(trim($data), $parse);
		}	
...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
      ...
      $category_tree_array = tep_get_category_tree($categories['categories_id'], $spacing . '&nbsp; &rarr; &nbsp;', $exclude, $category_tree_array);
    }
  ...
  function tep_draw_products_pull_down($name, $parameters = '', $exclude = '') {
    ...
    $select_string .= '>';
      ...
      $customers_groups_query = tep_db_query("SELECT customers_group_name, customers_group_id 
	...
	$products_query = tep_db_query("SELECT p.products_id, 
									...
									LEFT JOIN ". TABLE_PRODUCTS_GROUPS ." pg ON pg.products_id = pd.products_id
    ...
    while ($products = tep_db_fetch_array($products_query)) {
     ...
     if (!in_array($products['products_id'], $exclude)) {
		...
		foreach($all_groups as $sdek => $sdev) { 
             ...
             if (!in_array($products['products_id'].":".$sdek, $all_groups)) {
  ...
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    if (!$address_format_id) $address_format_id=2;
...
// Text Mode
  ...
  function tep_get_products_info_alt($product_id, $language_id) {
  ...
  function tep_get_products_description($product_id, $language_id) {
	...
	// # encode html entities such as &deg; and various other entities.
	$products_description = htmlentities($product['products_description'],ENT_NOQUOTES,'UTF-8');
	$products_description = str_replace(array('&lt;','&gt;'),array('<','>'), $products_description);
...
// Sets the status of a product
  function tep_set_product_status($products_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '1', products_last_modified = now() where master_products_id = '" . (int)$products_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '0', products_last_modified = now() where master_products_id = '" . (int)$products_id . "'");
    } else {
      return -1;
    }
  }
...
// Sets the status of a newsletter status
//  function tep_set_newsletter_status($subscribers_id, $status) {
  function tep_set_newsletter_status($subscribers_id, $status) {
	...
	////
// Sets the status of a newsletter status
  function tep_set_blacklist_status($subscribers_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_SUBSCRIBERS . " set subscribers_blacklist = '1' where subscribers_id = '" . (int)$subscribers_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_SUBSCRIBERS . " set subscribers_blacklist = '0' where subscribers_id = '" . (int)$subscribers_id . "'");
    } else {
      return -1;
    }
  }
...
////
// By MegaJim
  function tep_cfg_save_file($validate, $key_value, $key = '') {
    $name = ((tep_not_null($key)) ? "save_file[$key]" : 'save_file');
    return '<input type="hidden" name="'.$name.'[validate]" value="'.$validate.'"><textarea name="'.$name.'[content]" rows="8" cols="20"></textarea>';
  }

  function tep_cfg_password($key_value, $key = '') {
    $name = ((tep_not_null($key)) ? 'configuration[' . $key . ']' : 'configuration_value');
    return '<input type="password" name="'.$name.'" value="'.htmlspecialchars($key_value).'">';
  }

  function tep_display_password($val) {
...
//++++ QT Pro: Begin Changed code
      ...
      while ($order = tep_db_fetch_array($order_query)) {
        ...
        tep_db_query("UPDATE " . TABLE_PRODUCTS . "  SET products_quantity = products_quantity + " . $product_stock_adjust . ", products_ordered = products_ordered - " . (int)$order['products_quantity'] . ", last_stock_change = NOW() WHERE products_id = '" . (int)$order['products_id'] . "'");
//++++ QT Pro: End Changed Code
      }
    }
  ...
  function tep_remove_supply_request($supply_request_id) {
    tep_db_query("DELETE FROM " . TABLE_SUPPLY_REQUEST . " WHERE supply_request_id = '" . (int)$supply_request_id . "'");
    tep_db_query("DELETE FROM " . TABLE_SUPPLY_REQUEST_PRODUCTS . " WHERE supply_request_id = '" . (int)$supply_request_id . "'");
    tep_db_query("DELETE FROM " . TABLE_SUPPLY_REQUEST_PRODUCTS_ATTRIBUTES . " WHERE supply_request_id = '" . (int)$supply_request_id . "'");
    tep_db_query("DELETE FROM " . TABLE_SUPPLY_REQUEST_STATUS_HISTORY . " WHERE supply_request_id = '" . (int)$supply_request_id . "'");
    tep_db_query("DELETE FROM " . TABLE_SUPPLY_REQUEST_TOTAL . " WHERE supply_request_id = '" . (int)$supply_request_id . "'");
  }
...
// Output the tax percentage with optional padded decimals
  ...
  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address) {
    ...
    $message = new email(array('X-Mailer: IntenseCart eCommerce'));
...
// ########## END - password reset encrypt ##########


// ########## Whos Online Enhancment ##########
...
// ########## END - Whos Online Enhancment ##########
// START FCKEditor for images
  function fck_filter_image ($products_image_name) {
  	$products_image_name = htmlspecialchars(stripslashes($products_image_name));
		$products_image_name_test = strstr($products_image_name, 'src=');
		if ($products_image_name_test == $products_image_name) {
		    return $products_image_name;
		}
		$products_image_name = preg_replace('/src=&quot;/', '', $products_image_name_test);
		$products_image_name_end = strstr($products_image_name, '&quot;');
		$products_image_name = preg_replace('/'.$products_image_name_end.'/', '', $products_image_name);
		$products_image_name = preg_replace('/'.DIR_WS_CATALOG . DIR_WS_IMAGES.'/', '', $products_image_name);
		return $products_image_name;
	}
	
	function fck_filter_save_image ($products_delete_image) {
  	$products_delete_image = htmlspecialchars(stripslashes($products_delete_image));
		$products_delete_image = preg_replace('/&lt;img /', '', $products_delete_image);
		$products_delete_image = preg_replace('/src=/', '', $products_delete_image);
		$products_delete_image = preg_replace('/alt=&quot;&quot; /', '', $products_delete_image);
		$products_delete_image = preg_replace('/&quot;/', '', $products_delete_image);
		$products_delete_image = preg_replace('/\/&gt;/', '', $products_delete_image);
		$products_delete_image = preg_replace(DIR_WS_CATALOG . DIR_WS_IMAGES, '', $products_delete_image);
		return $products_delete_image;
	}
// END

  function tep_time_format($time,$tz) {
    $time=preg_replace('/.*\s/','',$time);
    if (DISPLAY_TIME_12H=='True') {
      list($h,$m,$s) = explode(':',$time);
      if ($h>12) { $h-=12; $ap='p'; }
      else if ($h==0) { $h=12; $ap='p'; }
      else { $h+=0; $ap='a'; }
      $time="$h:$m:$s$ap";
    }
    if ($tz) $time.="&nbsp;$tz";
  ...
  function tep_get_products_extra($product_id, $key) {
    $row=tep_db_fetch_array(tep_db_query("SELECT products_extra_value FROM products_extra WHERE products_id='$product_id' AND products_extra_key='$key'"));
    return isset($row['products_extra_value'])?$row['products_extra_value']:NULL;
  }
  function tep_set_products_extra($product_id, $key, $value) {
    if (isset($value)) {
      tep_db_query("REPLACE INTO products_extra (products_id,products_extra_key,products_extra_value) VALUES ('$product_id','$key','".tep_db_input($value)."')");
    } else {
      tep_db_query("DELETE FROM products_extra WHERE products_id='$product_id' AND products_extra_key='$key'");
    }
  }
  ...
  function tep_get_product_models($products_id) {
    global $modelsCache;
    if (!isset($modelsCache[$products_id])) tep_get_models_info($products_id);
    return $modelsCache[$products_id]['attributes'];
  }

  function tep_get_product_model_fields($products_id,$model_id) {
    global $modelsCache;
    if (!isset($modelsCache[$products_id])) tep_get_models_info($products_id);
    return $modelsCache[$products_id]['fields'][$model_id];
  }

  function tep_get_product_options($products_id) {
    global $modelsCache;
    if (!isset($modelsCache[$products_id])) tep_get_models_info($products_id);
    return $modelsCache[$products_id]['options'];
  }

	function tep_get_models_info($products_id) {
		global $modelsCache;
		$models = array();
		$msort = array();
		$options = array();
		$pfields = array();
		$qry = tep_db_query("SELECT a.options_id,
					    a.options_values_id,
					    a.options_sort,
					    a.options_values_sort,
					    a.options_image,
					    p.* 
					FROM ".TABLE_PRODUCTS." p 
					LEFT JOIN ".TABLE_PRODUCTS_ATTRIBUTES." a ON a.products_id = p.products_id 
					WHERE p.master_products_id = '$products_id' 
					AND p.products_id != '$products_id' 
					ORDER BY a.options_sort, a.options_values_sort
					");

		while($row=tep_db_fetch_array($qry)) {

			if(!isset($pfields[$row['products_id']])) {
	
				$row['products_image_list'] = array($row['products_image']);

				for ($i=1;$i<=4;$i++) { 
					$row['products_image_list'][$i] = $row['products_image_xl_'.$i];
				}

				$pfields[$row['products_id']] = $row;
    		}

			if($row['options_id'] && !isset($options[$row['options_id']])) {
				$options[$row['options_id']] = array();
			}

			if(!isset($models[$row['products_id']])) {
				$models[$row['products_id']] = array();
			}

			if(!isset($msort[$row['products_id']])) {
				$msort[$row['products_id']] = array();
			}

			if($row['options_id']) {
				$models[$row['products_id']][$row['options_id']] = $row['options_values_id'];
				$options[$row['options_id']][$row['options_values_id']] = $row;
				$msort[$row['products_id']][$row['options_id']] = $row['options_values_sort'];
			}
		}
    	...
    	asort($msort);
	    foreach($msort AS $pid=>$srt) {
			$smodels[$pid]=&$models[$pid];
		}

		return $modelsCache[$products_id] = array('options' => $options,
												  'attributes' => $smodels,
												  'fields'=>$pfields);
		}
	...
	function tep_build_cat_pull_down(&$cat_select,$prfx,$cat_tree=NULL,$pr='') {
		...
		if(!isset($cat_tree)) { 
			$cat=tep_get_category_info(); $cat_tree=$cat['tree']; 
		}
...
include(DIR_FS_FUNCTIONS.'refund_functions.php');
?>
