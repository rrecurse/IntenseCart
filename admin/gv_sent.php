...
<?php
  $gv_query_raw = "select c.coupon_amount, c.coupon_code, c.coupon_id, et.sent_firstname, et.sent_lastname, et.customer_id_sent, et.emailed_to, et.date_sent, c.coupon_id, o.orders_id, o.customers_name,o.date_purchased from " . TABLE_COUPONS . " c LEFT JOIN " . TABLE_COUPON_EMAIL_TRACK . " et ON c.coupon_id = et.coupon_id LEFT JOIN ". TABLE_ORDERS_PRODUCTS. " op ON c.purchase_ref=op.orders_products_id LEFT JOIN ".TABLE_ORDERS." o ON op.orders_id=o.orders_id WHERE c.coupon_type='G' ORDER BY c.coupon_id DESC";
//  $gv_query_raw = "select c.coupon_amount, c.coupon_code, c.coupon_id, et.sent_firstname, et.sent_lastname, et.customer_id_sent, et.emailed_to, et.date_sent, c.coupon_id from " . TABLE_COUPONS . " c LEFT JOIN " . TABLE_COUPON_EMAIL_TRACK . " et ON c.coupon_id = et.coupon_id WHERE et.coupon_id!=0";
...
?>
                <td class="dataTableContent"><?php echo isset($gv_list['customers_name'])? $gv_list['customers_name'] : $gv_list['sent_firstname'] . ' ' . $gv_list['sent_lastname']; ?></td>
                ...
                <td class="dataTableContent" align="right"><?php echo tep_date_short(isset($gv_list['date_purchased'])?$gv_list['date_purchased']:$gv_list['date_sent']); ?></td>
