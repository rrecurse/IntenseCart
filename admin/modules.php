...
//          tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '" . $value . "' where configuration_key = '" . $key . "'");
	  @unlink(FILENAME_CONFIG_CACHE);
        ...
        if (isset($_POST['save_file'])) while (list($key, $value) = each($_POST['save_file'])) {
	  if (preg_match('/\S/',$value['content']) && defined($key)) {
	    if (!isset($value['validate']) || preg_match($value['validate'],$value['content'])) {
	      if ($save_file_fd=fopen(DIR_FS_CATALOG_LOCAL.preg_replace('|.*/|','',constant($key)),'w')) {
	        fwrite($save_file_fd,$value['content']);
	        fclose($save_file_fd);
	      }
	    }
	  }
