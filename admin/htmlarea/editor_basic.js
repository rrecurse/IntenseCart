...
//  ['custom1','custom2','separator'],
    ['translation','separator','custom3','separator'],
...
this.btnList = {
    ...
    "custom3":           ['custom3',         ' Add a Tab Description ',  'editor_action(this.id)',  'ed_custom.gif'],
   // end: custom buttons
...
// end insert by lvn
  ...
  if (fileLink) { editor_insertHTML(objname, unescape( fileLink) ); }
    ...
    alert(" IntenseCart WYSIWYG Editor ");
  }
  ...
  // Custom3
    ...
    editor_insertHTML(objname, "&nbsp;<newtab><tabname>TAB NAME HERE</tabname><tabtext>TAB TEXT AND CONTENT GOES HERE </tabtext></newtab>&nbsp;");
  }
...
function editor_setmode(objname, mode) {
  ...
  //src="' +_editor_url+ 'popups/blank.html" This is put on the previous line in order to avoid warning in SSL mode. But back button does not work anymore!
