  ...
  switch ($_GET['action']) {
    ...
    case 'save':
		...
		for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
			...
			$insert_sql_data = array('returns_status_id' => $returns_status_id, 'language_id' => $language_id);
...
<?php
      ...
      $default_status_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'DEFAULT_RETURN_STATUS_ID'");
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
    ...
    if ($default_status['configuration_value'] == $orders_status['returns_status_id']) {
...
<?php
  ...
  switch ($_GET['action']) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'delete':
      ...
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>' . $oInfo->returns_status_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_RETURNS_STATUS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->returns_status_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
