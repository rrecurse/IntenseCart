   ...
   private function collapseWhitespace($s) {
    ...
    private function _httpPost(array $parameters)
    {
        ...
        switch ($url['scheme']) {
            case 'https':
                $scheme = 'https://';
                $port = !isset ($port) ? 443 : $port;
                break;
            default:
                $scheme = 'http://';
                $port = !isset ($port) ? 80 : $port;
        }
		...
		// # getting random errors, traced them to the following article:
		// # http://stackoverflow.com/questions/14304187/getting-internal-error-on-amazon-marketplace-api-requests
        ...
        if (isset ($_config) && $_config['ProxyHost'] != null && $_config['ProxyPort'] != -1){
