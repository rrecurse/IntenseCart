...
// # error_log(print_r('from /common/service/newegg_inventory - time now is ' . date('Y-m-d H:i:s'),TRUE));
...
define ('LOCKFILE', DIR_FS_SITE."cache/newegg_orders_{$Region}.lock");
define ('FILE_RUNTIME', DIR_FS_SITE."cache/newegg_orders_{$Region}.runtime");
define ('FILE_DUMP', DIR_FS_SITE."cache/newegg_orders_{$Region}.dump");
define ('FILE_SUBMIT', DIR_FS_SITE."cache/newegg_orders_{$Region}.submit");
define ('FILE_COOKIE', DIR_FS_SITE."cache/newegg_cookie_$Region");
...
if (!is_file (FILE_DUMP)) {
    // # Retrieve Newegg API credentials.
    ...
    // # Get locally updated orders, and send them to Newegg.
    if ($Status = Update_Newegg ($DB, $Creds, $PollTime, $Region)) {
        // # Remove lock file and return the error code.
        unlink (LOCKFILE);
echo "Could not update Newegg, error code $Status\n";
        return $Status;
    }

    // # Retrieve orders from Newegg.
    if (($Status = Poll_Orders ($Region, $Creds, $PollTime)) > 0) {
echo "Could not retrieve orders from Newegg, error status: $Status\n";
...
// # fix this to be more detective of the Newegg MFN / SBN region. name profile in db to Newegg.$Region-FBA and detect for '-FBA'
// # added additional crontab under user apache with region us-fba to compensate.
$Post = array (
    "csv_file" => "@".FILE_DUMP,
    'profile_import' => 'Newegg'.strtoupper($Region),
    'imp_action' => 'Import'
);
...
/*
 # Retrieves new and updated orders from the database, and sends them to Newegg.
 #
 ...
 # @return int
 */
function Update_Newegg (mysqli $DB, $Creds, $PollTime, $Region) {
    ...
    if ($Region == 'ca') {
		...
		$ServiceURL =  "https://api.newegg.ca/marketplace/ordermgmt/order/orderinfo?sellerid=".$Creds['seller_id']."&version=304";
    } else { 
		$ServiceURL =  "https://api.newegg.com/marketplace/ordermgmt/order/orderinfo?sellerid=".$Creds['seller_id']."&version=304";
    }
    ...
    if ($Status = Update_NeweggRMA ($Service, $Creds, $RMAs, $Region)) {
        return $Status;
    }

    // # Send cancelled orders as 'OrderAcknowledgement'
    if ($Status = Update_NeweggOrderStatus ($Service, $Creds, $Cancels, 'Cancellations')) {
        return $Status;
    }

    // # Send shipped orders as 'OrderFulfillment'
    if ($Status = Update_NeweggShip ($Service, $Creds, $Updates)) {
        return $Status;
    }
...
function Get_ShippedOrders ($DB, $LastRun) {
    ...
    //error_log(print_r('from /common/service/newegg_orders - $LastRun is: ' . $LastRun . ' time now is ' . date('Y-m-d H:i:s'),TRUE));
...
WHERE
    o.customers_email_address LIKE "%newegg.com%"
    AND o.orders_status = 3
    AND o.last_modified >= '$LastRun'
    AND r.refund_amount IS NULL
    AND o.orders_source NOT LIKE '%Newegg-SBN%'
...
WHERE
    o.customers_email_address LIKE "%@marketplace.newegg.%"
...
OutSQL;
    ...
    while ($Row = $Res->fetch_array()) {
        ...
        if($Row['products_exchanged'] == 1) {   
            ...
            // # If Newegg requires the price adjustment data, or to enable support for partial refunds.
...
//if(!empty($Data)) error_log(print_r($Data,1), 1, 'chrisd@zwaveproducts.com', 'Subject: Data dump from newegg_orders php Get_RMAOrders function');
 ...
 * @return int
 */
function Update_NeweggShip ($Service, $Creds, $Data) {
...
';
    ...
    $Feed = sprintf ($FeedTemplate, $Creds['seller_id'], $OrderData);
 ...
 * @return int
 */
function Update_NeweggOrderStatus ($Service, $Creds, $Data, $Type) {
    // If no updates, return successfully.
    if (empty ($Data)) {
        return 0;
    }
 ...
 * @return int
 */
function Update_NeweggRMA ($Service, $Creds, $Data, $Region) {
...
';
    ...
    $Feed = sprintf ($FeedTemplate, $Creds['seller_id'], $OrderData);
//echo "RMA:\n";
//error_log(print_r($Feed,1), 1, 'chrisd@zwaveproducts.com', 'Subject: RMA from newegg_orders');
    // # Return 0 for debug purposes.
...
function Submit_Feed ($Type, $Service, $Creds, $Feed, $Name) {
    ...
    $Request->setMerchant ($Creds['seller_id']);
    ...
    for ($Run = 0; $Run < 5; $Run++) {
        try {
            ...
            if ($ex->getErrorCode () == 'RateLimitError') {
                sleep (SUBMIT_TIMEOUT);
                continue;
            }
...
function Poll_Orders ($Region, $Creds, $PollTime) {
    ...
    // # Canada
    if ($Region == 'ca') {
        $ServiceURL = "https://api.newegg.ca/Orders/2011-01-01";
    // # Europe
    } elseif($Region == 'de' || $Region == 'es' || $Region == 'fr' || $Region == 'it' || $Region == 'uk') {
        $ServiceURL = "https://api.newegg.com/Orders/2011-01-01";
    // # Japan
    } elseif($Region == 'jp') {
        $ServiceURL = "https://api.newegg.jp/Orders/2011-01-01";
    // # India
    } elseif($Region == 'in') {
        $ServiceURL = "https://api.newegg.in/Orders/2011-01-01";
    // # China
    } elseif($Region == 'cn') {
        $ServiceURL = "https://api.newegg.com.cn/Orders/2011-01-01";
    // # United states
    } else {
        $ServiceURL = "https://api.newegg.com/Orders/2011-01-01";
    }
    ...
    $Request->setSellerId ($Creds['seller_id']);
    ...
    foreach ($OrderData as $Order) {
        ...
        foreach ($ItemData as $Item) {
            ...
            if ($Channel === 'SBN'){
                ...
                $orderSource = 'Newegg-SBN_'.strtoupper($Region);

            } else {

                $orderSource = 'dbfeed_newegg_'. $Region;
            }
            ...
            $Temp = array (
                ...
                "payment-method" => "payment_neweggSeller",
    ...
    if ($Region == 'ca') {
        $ServiceURL =  "https://api.newegg.ca";
    } elseif($Region == 'de' || $Region == 'es' || $Region == 'fr' || $Region == 'it' || $Region == 'uk') {
        $ServiceURL =  "https://api.newegg.com";
    } elseif($Region == 'jp') {
        $ServiceURL =  "https://api.newegg.jp";
    } elseif($Region == 'in') {
        $ServiceURL =  "https://api.newegg.in";
    } elseif($Region == 'cn') {
        $ServiceURL =  "https://api.newegg.com.cn";
    } else { 
        $ServiceURL =  "https://api.newegg.com";
    }
    ...
    $Service = new NeweggMarketplace_Client ($Creds['access_key'], $Creds['secret_key'], $Config, APPLICATION_NAME, APPLICATION_VERSION);

    // # Mark orders as imported in Amazon MWS.
    if ($Status = Update_NeweggOrderStatus ($Service, $Creds, $OrderImport, 'Acknowledgements')) {
        return $Status;
    }
