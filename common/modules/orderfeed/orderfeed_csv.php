...
class orderfeed_csv extends IXmodule {
	...
	var $rawOrderFields = "
      ...
      orders_source, Orders Source,		none/unique
      orders_status, Orders Status,     none/unique
    ";
...
//$order->addProduct([fake id],1,array(),$diff);
		...
		if ($orders_source) 
			$order->info['orders_source'] = $orders_source;
        if ($orders_status){
            $order->setStatus($orders_status);
        }

		$order->saveOrder();

		// # Save the order-item-id from Amazon in the orders_items_refs table.
		foreach ($items as $item) {
	...
	function getOrderData (&$order, $field) {
		...
		switch ($keyX[0]) {
			...
			case 'orders';
                switch ($keyX[1]){
                    case 'source' :
    	    			return $order->info['orders_source'];
                    case 'status':
                        return $order->info['orders_status'];
                }
                break;
			case 'd' :
