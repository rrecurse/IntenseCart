...
class dbfeed_thefind extends IXdbfeed {
	var $category_path_cache, $feed, $filename, $products;

  function dbfeed_thefind () {
        parent::ixdbfeed();
        $this->category_separator = "/";
        $this->cols_separator = "\t";
		$this->filename = DIR_FS_SITE_CATALOG . 'pub/' . 'thefind.txt';
  }
  ...
  function _get_text2($text, $length = 0) {
	...
	function buildFeed() {
	    ...
	    while (list(,$product_row) = each($this->feed_products)) {
			...
			"CPC Rate"					=>	$this->_get_text2(''), // # optional
			"User_rating"				=>	$this->_get_text2(''), // # optional
			"Review_count"				=>	$this->_get_text2(''), // # optional
			"Video_title"				=>	$this->_get_text2(''), // # optional
			"Video_link"				=>	$this->_get_text2(''), // # optional
			);
	...
	function listConf() {
    	return array('shipping' => array('title'=>'Default Shipping Cost','desc'=>'Default shipping cost','default'=>'0'),
					 'ftp_host' => array('title'=>'TheFind FTP Host','desc'=>'HTTP Host for feed upload','default'=>''),
					 'ftp_user' => array('title'=>'TheFind FTP User','desc'=>'','default'=>''),
					 'ftp_pass' => array('title'=>'TheFind FTP Password','desc'=>'','default'=>''),
					);
	}
...
?>
		<table border="0" cellspacing="2" cellpadding="0">
			...
			<tr>
				<td valign="top">TheFind Shopping Category:</td>
				<td>
					<table border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td><?php echo tep_draw_input_field('dbfeed_extra['.get_class($this).'][category]',$xflds['category'],'id="thefind_cat_fld"');?></td>
							<td><div class="helpicon" onmouseover="ddrivetip('<font class=featuredpopName><b style=\'white-space:nowrap\'>TheFind Shopping Category</b></font><br><br>The categorization field should be filled out based on TheFind Shopping\'s Taxonomy (category structure), with each level of taxonomy separated by a \'>\' (e.g. Clothing > Accessories > Sunglasses). To determine where an item should be categorized, you visit the TheFind Shopping catagory tool and search for the product (or a similar product). When you find it on their site, look to the top left of the page. You\'ll see a \'breadcrumb\' or list of the categories that product falls into.')" onMouseout="hideddrivetip()"> </div></td>
							<td>[<a href="http://support.google.com/merchants/bin/answer.py?hl=en&answer=1705911" target="_blank">TheFind</a>]</td>
						</tr>
					</table>
...
<?php if($cat_lkup) { ?>
	...
	<br>
	<?php echo tep_draw_pull_down_menu('thefindbase_cat_pickup',array_merge(array(array('id'=>'','text'=>'---Pick a Category---')),$cat_lkup),'','onChange="if (this.value) {$(\'thefind_cat_fld\').value=this.value; this.options[0].selected=true; }"'); ?>
<?php } ?>
</td></tr></table>
<?php
  }
